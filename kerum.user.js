...
//   Fix minor avoid local-time-system abuse; // @require  	  http://ajax.googleapis.com/ajax/libs/jquery/1.6.2/jquery.min.js
//
// -/!latestupdate---
// ==/UserScript==
/*
...
function wew(data){
		//alert(data);
			var capcay = $(data).find('label[for="humanverify"]').html();
...
function ajax_buildcapcay(reply_html){
  ...
  if(isUndefined(reply_html)){ // is there ret from XHR :: reply_html
    ...
    if( rets ) {
        ...
        if(!gvar.settings.recaptcha) create_kaskus_capcay(rets,reply_html);
...
function create_kaskus_capcay( rets , replax){
  ...
  if(isUndefined(reply_html)){ // is there ret from XHR :: reply_html
    ...
    if(!gvar.user.isDonatur){
      ...
      if( gvar.settings.recaptcha ){
        ...
        ce,fld = ["humanverify[hash]","humanverify[input]"];
        for(var i=0; i<fld.length; i++){
            ce=$D(fld[i]);
            if(ce) spost ='&'+ce.getAttribute('name')+'='+encodeURIComponent(ce.value) + spost;
        }
...
function loadLayer_reCaptcha(){
    ...
    var is_capcay_filled = function(tgt){
        if($D('input[name="humanverify[input]"]') && !$D('input[name="humanverify[input]"]').value){
          alert('Belum Isi Image Verification');
          try{if(typeof(tgt)=='object') tgt.focus()}catch(e){}
          return false;
        }else{
